...
FreeBSDで運用しているPRIMERGY TX1310 M5で、ATA Status Error がログに大量に出ていたのを見つけました。
...
が数秒おきに繰り返されていました。ディスク故障判断から解決を探った記録になります。

結果として、冷却能力不足と　BIOS設定(Fan制御を AUTO->FULL に）変えて暫定的に解決（騒音問題が浮上)しました。
...
当該エラーを吐き出した HDD は 運用環境温度を一時的にでも　75 ℃　(70°Ｃまでが保証範囲)　超過してしまったので HGST の銘品でしたが交換決定とあいなりました。容量不足もあったので 8TB の IronWolf-PRO になります 💸 


そのトラブルを記録した日はちょうど停電作業にあたる土曜日で、出社している人がいないオフィスにある機材でした。
翌週（この記事を書いた日）の土曜日に交換を兼ねて調査と運用温度状況の改善が必要でした。
...
過去1年間のRRDを見ると、通常 35℃ 前後、最大 56℃ という記録です。
...
- 最初からファン速度を FULL に 上げておけばよかったのですが、こうなると富士通製のサーバとしてはラックタイプに近い騒音発生量です。サーバルームであれば問題ないでしょうがこの手のタワー型は事務所に置かれがちです。騒音問題が議題になるかもしれません。
